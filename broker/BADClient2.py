...
def test_client():
    def on_result(channelName, subscriptionId, deliveryTime, results):
        ...
        horizontal_line='-'*100
        try:
            express_update=requests.get(expressURL+'/update',params={'statements':horizontal_line})
        except:
            pass
    client = BADClient(brokerUrl=brokerUrl)
    #client.register(sys.argv[1], 'yusuf', 'yusuf')
    client.register('baduser', 'baduser', 'baduser')
    if client.login():
        if client.subscribe('allEmergenciesChannel',['shooting'], on_result):
